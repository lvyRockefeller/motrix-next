...
describe('formatLimitBadge', () => {
  ...
  it('returns "512K" for "512K"', () => {
    expect(formatLimitBadge('512K')).toBe('512K')
  })

  it('returns "10M" for "10M"', () => {
    expect(formatLimitBadge('10M')).toBe('10M')
  })

  it('returns "1G" for "1G"', () => {
    expect(formatLimitBadge('1G')).toBe('1G')
  })

  it('converts "1024K" to "1M" for cleaner display', () => {
    expect(formatLimitBadge('1024K')).toBe('1M')
  })

  it('converts "1024M" to "1G" for cleaner display', () => {
    expect(formatLimitBadge('1024M')).toBe('1G')
  })

  it('does not convert non-round values (e.g., "500K" stays "500K")', () => {
    expect(formatLimitBadge('500K')).toBe('500K')
  })
