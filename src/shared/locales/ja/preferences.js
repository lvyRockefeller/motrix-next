export default {
  ...
  'hardware-rendering-hint': 'より多くの Linux GPU ドライバーとの互換性のため、デフォルトではソフトウェアレンダリングを使用します。有効にすると DMA-BUF ハードウェアアクセラレーションが有効になり UI がスムーズになりますが、一部の GPU（NVIDIA プロプライエタリ、Intel + Wayland、Raspberry Pi など）で白画面やクラッシュが発生する場合があります。有効化後にアプリがクラッシュした場合、次回起動時に自動的に無効になります。',
  'hardware-rendering-restart-confirm': 'GPU ハードウェアアクセラレーションは WebKitGTK DMA-BUF レンダラーを使用し、プロセス開始前に設定する必要があるため、この変更には再起動が必要です。有効化によりアプリが正常に起動できない場合、次回起動時にこのオプションは自動的に無効になります。ダウンロードタスクは影響を受けません。',
