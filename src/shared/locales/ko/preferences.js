export default {
  ...
  'hardware-rendering-hint': '더 넓은 Linux GPU 드라이버 호환성을 위해 기본적으로 소프트웨어 렌더링이 사용됩니다. 활성화하면 DMA-BUF 하드웨어 가속이 활성화되어 UI가 부드러워지지만, 일부 GPU(예: NVIDIA 독점 드라이버, Intel + Wayland, Raspberry Pi)에서 흰 화면이나 충돌이 발생할 수 있습니다. 활성화 후 앱이 충돌하면 다음 실행 시 자동으로 비활성화됩니다.',
  'hardware-rendering-restart-confirm': 'GPU 하드웨어 가속은 프로세스 시작 전에 구성해야 하는 WebKitGTK DMA-BUF 렌더러를 사용하므로 이 변경에는 재시작이 필요합니다. 활성화로 앱이 정상적으로 시작되지 않으면 다음 실행 시 이 옵션이 자동으로 비활성화됩니다. 다운로드 작업은 영향을 받지 않습니다.',
